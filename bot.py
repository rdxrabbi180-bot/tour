# bot.py
# এটাই মূল ফাইল, এটা রান করলেই বট চালু হবে
...
from config import BOT_TOKEN, ADMIN_IDS, PAYMENT_INFO

# লগ দেখানোর জন্য (এরর হলে টার্মিনালে দেখা যাবে)
logging.basicConfig(
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/myregistrations - আপনার রেজিস্ট্রেশন স্ট্যাটাস দেখুন\n"
...
async def list_tournaments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in tournaments:
        approved = db.count_approved_slots(t["id"])
        text = (
            f"🏆 {t['name']}\n"
...
# ============ রেজিস্ট্রেশন কথোপকথন (Conversation) ============

async def register_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ইউজার 'রেজিস্টার করুন' বাটনে ক্লিক করলে এটা চালু হবে"""
    ...
    approved = db.count_approved_slots(tournament_id)
...
async def get_p4_uid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tournament = db.get_tournament(context.user_data["tournament_id"])
    await update.message.reply_text(
        f"এখন পেমেন্ট করুন:\n{PAYMENT_INFO}\n"
...
async def get_trx_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reg_id = db.create_registration(reg_data)
    ...
    # Admin-কে নোটিফাই করা
    tournament = db.get_tournament(reg_data["tournament_id"])
    admin_text = (
...
async def approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    slot_number = db.count_approved_slots(reg["tournament_id"]) + 1
    db.approve_registration(reg_id, slot_number)
...
async def pending_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin সব pending রেজিস্ট্রেশন দেখতে পারবে"""
    if not is_admin(update.effective_user.id):
        return
...
async def setroom_pass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.set_room_details(tournament_id, room_id, room_password)
    tournament = db.get_tournament(tournament_id)
    approved_regs = db.get_approved_registrations(tournament_id)
